...
int main (int argc, char** argv) {
	...
	if (first_var[0]=="ele_pt") {

		TTreeReaderValue<double>  ele_variable( ele_reader, first_var[0].c_str() );

		if (second_var[0] == "ele_eta") {
			
			TTreeReaderValue<double>  et_eta_variable( ele_reader, second_var[0].c_str() );

			//Loop over the events
			while ( ele_reader.Next() ) {

				h2->Fill(*ele_variable, abs(*et_eta_variable));
		...
		if (second_var[0] != "ele_eta") {
			...
			//No need to create a mt_variable: the trigger is an &&
			TTreeReaderValue<double>  et_variable( ele_reader, second_var[0].c_str() );
						
			//Loop over the events
			while ( ele_reader.Next() ) {

				h2->Fill(*ele_variable, *et_variable);

			}//end of loop over the events	
		}
	}

	if (second_var[0]=="ele_pt") {

		TTreeReaderValue<double>  ele_variable( ele_reader, second_var[0].c_str() );

		if (first_var[0] == "ele_eta") {
			
			TTreeReaderValue<double>  et_eta_variable( ele_reader, first_var[0].c_str() );

			//Loop over the events
			while ( ele_reader.Next() ) {

				h2->Fill(abs(*et_eta_variable), *ele_variable);
		...
		if (first_var[0] != "ele_eta") {
			...
			//No need to create a mt_variable: the trigger is an &&
			TTreeReaderValue<double>  et_variable( ele_reader, first_var[0].c_str() );
						
			//Loop over the events
			while ( ele_reader.Next() ) {

				h2->Fill(*et_variable, *ele_variable);
	...
	if (first_var[0]=="mu_pt") {

		TTreeReaderValue<double>  mu_variable( mu_reader, first_var[0].c_str() );

		if (second_var[0] == "mu_eta") {
			
			TTreeReaderValue<double>  mt_eta_variable( mu_reader, second_var[0].c_str() );

			//Loop over the events
			while ( mu_reader.Next() ) {

				h2->Fill(*mu_variable, abs(*mt_eta_variable));
		...
		if (second_var[0] != "mu_eta") {
			...
			//No need to create a mt_variable: the trigger is an &&
			TTreeReaderValue<double>  mt_variable( mu_reader, second_var[0].c_str() );
						
			//Loop over the events
			while ( mu_reader.Next() ) {

				h2->Fill(*mu_variable, *mt_variable);

			}//end of loop over the events	
		}
	}

	if (second_var[0]=="mu_pt") {

		TTreeReaderValue<double>  mu_variable( mu_reader, second_var[0].c_str() );

		if (first_var[0] == "mu_eta") {
			
			TTreeReaderValue<double>  mt_eta_variable( mu_reader, first_var[0].c_str() );

			//Loop over the events
			while ( mu_reader.Next() ) {

				h2->Fill(abs(*mt_eta_variable), *mu_variable);
		...
		if (first_var[0] != "mu_eta") {
			...
			//No need to create a mt_variable: the trigger is an &&
			TTreeReaderValue<double>  mt_variable( mu_reader, first_var[0].c_str() );
						
			//Loop over the events
			while ( mu_reader.Next() ) {

				h2->Fill(*mt_variable, *mu_variable);
