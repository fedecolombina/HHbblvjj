...
//c++ fiveCuts.cpp -o fiveCuts `root-config --cflags --glibs`
...
int main (int argc, char** argv) {
	...
	cout << "Inserisci il valore del terzo taglio sul jet: " << endl;
	cin >> third_cut;
	...
	vector<double> cuts {first_cut, second_cut, third_cut};
	...
	while (reader.Next()) {
		...
		for (int iVar = 0; iVar < first_jet.GetSize(); ++iVar) {
			...
			if (jets[3] > cuts[2] && jets[2] > cuts[1] && jets[1] > cuts[0] && mjj_array[5] > mjj_cut && 
			MET_value[iVar] > MET_cut) {
